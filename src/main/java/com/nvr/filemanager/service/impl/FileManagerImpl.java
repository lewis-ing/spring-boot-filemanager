package com.nvr.filemanager.service.impl;
...
import com.nvr.filemanager.api.lang.FileManagerException;
import com.nvr.filemanager.configuration.ConfigFileManager;
import com.nvr.filemanager.model.response.Item;
import com.nvr.filemanager.model.response.Item.Type;
import com.nvr.filemanager.service.FileManager;
import com.nvr.filemanager.utils.PermissionUtils;
...
public class FileManagerImpl implements FileManager {
	...
	public List<Item> list(String path) {
		LOG.debug("List path: Paths.get('{}', '{}'), onlyFolders: {}", configFileManager.getRepoBasePath(), path,configFileManager.isOnlyFolders());
		List<Item> items = Lists.newArrayList();

		try (DirectoryStream<Path> directoryStream = Files.newDirectoryStream(Paths.get(configFileManager.getRepoBasePath(), path))) {
	...
	public boolean copy(List<String> items, String newPath, String newFileName) {
		try {
			...
			for (String item : items) {
				Path path = newFileName == null ? Paths.get(configFileManager.getRepoBasePath(), item): Paths.get(".", newFileName);
				Path mpath = newpath.resolve(path.getFileName());
				LOG.debug("mv {} to {} exists? {}", path, mpath, Files.exists(mpath));
				if (Files.exists(mpath)) {
					throw new FileManagerException("File already exist");
				...
				mpath = newpath.resolve(newFileName == null ? path.getFileName() : Paths.get(".", newFileName).getFileName());
	...
	public String getContent(String item) {
		try {
			return FileUtils.readFileToString(Paths.get(configFileManager.getRepoBasePath(), item).toFile(),StandardCharsets.UTF_8);
		} catch (IOException ex) {
	...
	public boolean changePermission(List<String> items, String perms, String permsCode, boolean recursive) {
		try {
			for (String item : items) {
				LOG.debug("changepermissions path: {} perms: {} permsCode: {} recursive: {}", item, perms, permsCode,recursive);
			...
			try (FileSystem zipfs = FileSystems.newFileSystem(URI.create("jar:" + zip.toUri().toString()), env)) {
				for (String item : items) {
					...
					if (Files.isDirectory(realPath)) {
						...
						Path pathInZipFile = zipfs.getPath("/", realPath.toString().substring(configFileManager.getRepoBasePath().length() + destination.length()));
	...
	public boolean extract(String zip, String destination, String folderName) {
		...
		try {
			...
			try (FileSystem zipfs = FileSystems.newFileSystem(URI.create("jar:" + Paths.get(configFileManager.getRepoBasePath(), zip).toUri()), env)) {
	...
	public boolean store(MultipartFile multipartFile, String newItemPath) {
        ...
        try {
            ...
            if (filename.contains("..")) {
                ...
                throw new FileManagerException("Cannot store file with relative path outside current directory "+ filename);
            }
			Path newpath = Paths.get(configFileManager.getRepoBasePath(), newItemPath);
	...
	@Override
	public OutputStream files2Zip(List<String> paths, OutputStream baos) throws IOException {
		if(!CollectionUtils.isEmpty(paths)) {
	        try (ZipOutputStream zos = new ZipOutputStream(new BufferedOutputStream(baos))) {
	        	paths.stream().forEach(path->{
					FileSystemResource fileSystemResource = new FileSystemResource(getFile(path));
					ZipEntry zipEntry = new ZipEntry(fileSystemResource.getFilename());
					try {
						zos.putNextEntry(zipEntry);
						IOUtils.copy(fileSystemResource.getInputStream(), zos);
					} catch (IOException e) {
